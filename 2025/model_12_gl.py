...
def performance():
    ...
    value.loc[["A"]] = (
        ...
        + sw_to_proctetive() * resource_generative_outcome()
        + (1 - sw_to_proctetive()) * shock_effect_on_performance()
    )
    value.loc[["B"]] = (
        cab() * float(resources().loc["A"])
        + cbb() * float(resources().loc["B"])
        + resource_generative_outcome()
    )
    return value
...
@component.add(
    name="sw to proctetive",
    limits=(0.0, 1.0, 1.0),
    comp_type="Constant",
    comp_subtype="Normal",
)
def sw_to_proctetive():
    return 0
